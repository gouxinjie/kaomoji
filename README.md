...
> **原理**：站内的颜文字、Emoji、特殊符号、部首、花字、装饰长串，本质上都是**一段 Unicode 文本字符**，而非图片。它们之所以"键盘打不出"，是因为对应的码位（Code Point）没有标准按键，但浏览器和操作系统字体都能正常渲染。复制时写入剪贴板的就是这些字符本身的文本内容，因此能在微信、文档、评论区等任何支持 Unicode 的文本框里原样粘贴使用，不受图片或字体的限制。
