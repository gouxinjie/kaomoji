@echo off
rem 一键启动本地预览服务器（颜文字图鉴）
rem 依赖：本机需安装 Python 或 Node.js
chcp 65001 >nul
cd /d "%~dp0"
echo ============================================
echo   颜文字图鉴 · 本地预览
...
rem 优先使用 Python；否则尝试 Node.js
python --version >nul 2>&1
if %errorlevel%==0 (
    echo [启动] 使用 Python 启动服务器，端口 8000
...
if %errorlevel%==0 (
    echo [启动] 使用 Node.js 启动服务器，端口 8000
    echo [访问] http://localhost:8000/
    echo [停止] 按 Ctrl+C
    echo.
...
echo 请先安装 Python(https://python.org) 或 Node.js(https://nodejs.org)
echo.
pause >nul 2>&1
exit /b 1
